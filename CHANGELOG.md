...
- `/INSTALL` subcommand no longer upgrades already-satisfied dependencies and no 
  longer installs later versions from source, both of which should reduce 
  installation time.
...
- `/INSTALL` subcommand now works even if Git is not installed. (Now relies on 
  git4r package.)
